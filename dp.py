...
class Network():
	...
	def define_model(self):
		...
		self.train_summaries.append(tf.summary.scalar('learning_rate', learning_rate))
	...
	def train(self, train_samples, train_labels, *, data_iterator, iteration_steps):
		...
		with tf.Session(graph=tf.get_default_graph()) as session:
			...
			plt.plot(y,'b',lw = 1.5) # 蓝色的线
			#plt.plot(y,'ro') #离散的点
			plt.grid(True)
			plt.axis([0,60,0,0.6])
			plt.xlabel('iteration steps')
			plt.ylabel('accuracy')
			plt.title('Trian Accuracy')
			plt.show()
